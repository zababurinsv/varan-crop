import upload from '/static/html/components/component_modules/varan-upload/varan-upload.js'
...
import Crop from '/static/html/components/component_modules/varan-crop/varan-crop.js'
  ...
  class extends HTMLElement {
    constructor () {
      ...
      async function modules (obj) {
        Store['init'](obj)
          .then((obj) => {
            function actions (events) {
              if (obj['this'].getAttribute('parent') === events['detail']['task']['parent']) {
                obj['events'] = events
                obj['store']['state']['actions']['router'](obj)
              }
            }
            document.addEventListener('store', actions, false)
          })

        function mode (event) {
          if (!obj['parent']) {
            console.assert(false, 'установите parent', obj)
          }
          let name = obj['parent']
          if (name === event['detail']['slot']) {
            obj['this'].slot = event.detail['mode']
          }
        }
        function mutation (mutationList) {
          mutationList.forEach(function (mutation) {
            switch (mutation.type) {
              case 'attributes':
                switch (mutation.attributeName) {
                  case 'slot':
                    if (obj['this'].getAttribute('slot') === 'edit') {
                      colorLog(`${obj}<varan-upload-in>upload['getObject'](obj)<varan-upload>`, 'red', obj)
                      upload['getObject'](obj)
                        .then((obj) => {
                          matcher['database']['request']['functions']['getObject'](obj)
                            .then((obj) => {
                              if (!obj['get']) {
                                ...
                                if (!obj['get'][`${obj['slot']}`]) {
                                  console.assert(false, 'пришёл не правильный объект')
                                } else {
                                  let name = {}
                                  if (!obj['slot']) {
                                    name = obj['parent']
                                  ...
                                  switch (name) {
                                    ...
                                    case 'varan-catalog':
                                      ...
                                      break
                                    default:
                                      colorLog(`[<->]~~~~~~~add Slider`, 'red', obj)
                                      Slider['set'](obj)
                                        .then((obj) => {
                                          colorLog(`[<0>]slider ${obj}`, 'red', obj)
                                        })
                                      break
                                  }
                          ...
                          colorLog(`${obj}<varan-upload-out>upload['getObject'](obj)<varan-upload>`, 'red', obj)
                        })
                    }
                    break
                }
        ...
        document.addEventListener('mode', mode)
      }
    }
  })
